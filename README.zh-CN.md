...
- 提供可独立使用的工具输出压缩器：去除 ANSI、合并重复噪声、保留错误和尾部摘要，并可归档原文；
- 保留旧配置备份，支持查看状态和卸载。
...
| 压缩命令、测试日志和工具输出 | 通过 `--shrink-output` 或请求包装器启用 |
...
把命令输出通过标准输入交给控制器，结果可以直接作为模型上下文；它优先保留错误、路径、代码和结构化输出，只折叠连续重复行，并在超限时保留头部、重要行和尾部。需要审计或恢复时可同时保存原文：
...
`--execute-request` 也会在结果中返回 `stdout_compact`、`stderr_compact` 和压缩统计；设置 `output_archive_dir` 即可保留每次调用的精确原文。压缩是抽取式的，不是语义摘要，模型需要原文时应读取归档。
