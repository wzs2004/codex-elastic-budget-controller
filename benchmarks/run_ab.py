...
def run_trial(case, treatment, profile_name, controller_plan, output_dir, round_number, prices, timeout_seconds):
    profile = dict(PROFILES[profile_name])
    if controller_plan:
        profile["compact_token_limit"] = controller_plan["budget_actions"]["compact_token_limit"]
...
def main():
    ...
    for round_number in range(1, args.rounds + 1):
        for index, case in enumerate(cases):
            request = {key: case[key] for key in (
                "prompt", "estimated_input_tokens", "task_type", "complexity",
                "quality_risk", "expected_tool_calls", "expected_turns",
                "latency_sensitive", "reusable_prefix",
            ) if key in case}
            controller_plan = CONTROLLER.request_budget(request, POLICY, {})
            adaptive_profile = controller_plan["budget_actions"]["profile"]
            order = ("baseline", "adaptive") if (round_number + index) % 2 else ("adaptive", "baseline")
            for treatment in order:
                profile_name = "standard" if treatment == "baseline" else adaptive_profile
                row = run_trial(
                    case, treatment, profile_name,
                    controller_plan if treatment == "adaptive" else None,
                    output_dir, round_number,
    ...
    metadata = {
        ...
        "method": "paired alternating-order A/B; baseline standard; adaptive live controller preflight; same model/provider/prompt/schema/tools",
    }
